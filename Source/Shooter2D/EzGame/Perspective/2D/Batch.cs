...
namespace EzGame.Perspective.Planar
{
    public class Batch
    {
        ...
        public void Begin(Matrix Matrix, bool Backup = false)
        {
            Begin(SortMode, BlendState, SamplerState, DepthStencilState, RasterizerState, Effect, Matrix, Backup);
        }
        ...
        /// <param name="Matrix">Self-explanatory.</param>
        public void Begin(SpriteSortMode SortMode, Matrix Matrix, bool Backup = false)
        {
            Begin(SortMode, BlendState, SamplerState, DepthStencilState, RasterizerState, Effect, Matrix, Backup);
        }
        ...
        /// <param name="Matrix">Self-explanatory.</param>
        public void Begin(SpriteSortMode SortMode, BlendState BlendState, Matrix Matrix, bool Backup = false)
        {
            Begin(SortMode, BlendState, SamplerState, DepthStencilState, RasterizerState, Effect, Matrix, Backup);
        }
        ...
        {
            if (Origin.IsScale)
                Origin.Value = new Vector2(((Source.HasValue ? Source.Value.Width : Texture.Width)*Origin.Value.X),
                    ((Source.HasValue ? Source.Value.Height : Texture.Height)*Origin.Value.Y));
            if (!Begun) Begin();
            SpriteBatch.Draw(Texture, Bounds, Source, Color, Angle, Origin.Value, Effect, Layer);
        }

        public void Draw(Texture2D Texture, Vector2 Position, Rectangle? Source, Color Color, float Angle, Origin Origin,
            float Scale, SpriteEffects Effect = SpriteEffects.None, float Layer = 0)
        {
            if (Origin.IsScale)
                Origin.Value = new Vector2(((Source.HasValue ? Source.Value.Width : Texture.Width)*Origin.Value.X),
                    ((Source.HasValue ? Source.Value.Height : Texture.Height)*Origin.Value.Y));
            if (!Begun) Begin();
            SpriteBatch.Draw(Texture, Position, Source, Color, Angle, Origin.Value, Scale, Effect, Layer);
        }
        ...
        public void DrawString(string String, SpriteFont Font, Vector2 Position, Origin Origin, Color Fore,
            ...
            float Layer = 0)
        {
            ...
            if (!Begun) Begin();
            if (Back != null)
                SpriteBatch.DrawString(Font, String, (Position + Vector2.One), (Color) Back, Angle, Origin.Value, Scale,
                    Effect, ((Layer > 0) ? (Layer - .0000001f) : Layer));
            SpriteBatch.DrawString(Font, String, Position, Fore, Angle, Origin.Value, Scale, Effect, Layer);
        }

        public void DrawRectangle(Rectangle Bounds, Color? Fill = null, Color? Stroke = null, float Angle = 0,
            int Size = 1, float Layer = 0)
        {
            if (!Begun) Begin();
            if (Fill != null)
                Draw(Pixel(Color.White, true), Bounds, null, (Color) Fill, Angle, Origin.None, SpriteEffects.None, Layer);
            if ((Stroke == null) || (Size < 1)) return;
            Draw(Pixel(Color.White, true), new Rectangle(Bounds.X, (Bounds.Y - Size), Bounds.Width, Size), null,
                (Color) Stroke, Angle, Origin.None, SpriteEffects.None, Layer);
            Draw(Pixel(Color.White, true), new Rectangle((Bounds.X - Size), Bounds.Y, Size, Bounds.Height), null,
                (Color) Stroke, Angle, Origin.None, SpriteEffects.None, Layer);
            Draw(Pixel(Color.White, true), new Rectangle((Bounds.X + Bounds.Width), Bounds.Y, Size, Bounds.Height), null,
                (Color) Stroke, Angle, Origin.None, SpriteEffects.None, Layer);
            Draw(Pixel(Color.White, true), new Rectangle(Bounds.X, (Bounds.Y + Bounds.Height), Bounds.Width, Size), null,
                (Color) Stroke, Angle, Origin.None, SpriteEffects.None, Layer);
        }
    }
}