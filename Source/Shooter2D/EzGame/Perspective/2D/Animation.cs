...
namespace EzGame.Perspective.Planar
{
    public class Animation
    {
        ...
        public void Override(Origin Origin) { for (int i = 0; i < Frames.Length; i++) Frames[i].Origin = null; Origins = Origin; }
        ...
        public Animation(string Path, int Frames, bool Loop = false, float? Speed = null)
        {
            ...
            {
                Texture2D Texture = Textures.Get(Path + (Path.EndsWith("/") ? string.Empty : "/") + i);
        ...
        public Animation(string Path, int Frames, bool Loop = false, float? Speed = null, params Origin[] Origins)
        {
            ...
            {
                Texture2D Texture = Textures.Get(Path + (Path.EndsWith("/") ? string.Empty : "/") + i);
                Origin Origin = ((Origins.Length > i) ? Origin.Center : Origins[i]);
        ...
        public void Draw(Vector2 Position, Color Color, float Angle, float Scale, SpriteEffects Effect = SpriteEffects.None, float Layer = 0)
        { Draw(Globe.Batches[0], Position, Color, Angle, Scale, Effect, Layer); }
        public void Draw(Vector2 Position, Color Color, float Angle, Vector2 Scale, SpriteEffects Effect = SpriteEffects.None, float Layer = 0)
        { Draw(Globe.Batches[0], Position, Color, Angle, Scale, Effect, Layer); }
        public void Draw(Batch Batch, Vector2 Position, Color Color, float Angle, float Scale, SpriteEffects Effect = SpriteEffects.None, float Layer = 0)
        { Batch.Draw(Texture(), Position, null, Color, Angle, Origin(), Scale, Effect, Layer); }
        public void Draw(Batch Batch, Vector2 Position, Color Color, float Angle, Vector2 Scale, SpriteEffects Effect = SpriteEffects.None, float Layer = 0)
        { Batch.Draw(Texture(), Position, null, Color, Angle, Origin(), Scale, Effect, Layer); }

        public Texture2D Texture(uint? Index = null)
        {
            if (Frames == null) return null;
            if (Index == null) { if (Frames.Length > this.Index) return Frames[this.Index].Texture; }
        ...
        public Origin Origin(uint? Index = null)
        {
            ...
            if (Index == null) { if (Frames.Length > this.Index) return (Frames[this.Index].Origin ?? (Origins ?? Textures.Origin.Center)); }
        ...
        public class Frame
        {
            ...
            public Frame(Origin Origin) { this.Origin = Origin; }
            public Frame(Texture2D Texture) { this.Texture = Texture; }
            public Frame(Texture2D Texture, float Speed) { this.Texture = Texture; this.Speed = Speed; }
            public Frame(float Speed, Origin Origin = null) { this.Speed = Speed; this.Origin = (Origin ?? Origin.Center); }
            public Frame(Texture2D Texture, Origin Origin = null) { this.Texture = Texture; this.Origin = Origin; }
            public Frame(Texture2D Texture, float Speed, Origin Origin = null) { this.Texture = Texture; this.Speed = Speed; this.Origin = Origin; }
        }
    }
}