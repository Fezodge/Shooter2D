...
namespace EzGame.Perspective.Planar
{
    public class Raycaster
    {
        public Polygon CreateArea
        {
            get
            {
                var Area = new Polygon(new Line[Rays.Length]);
                for (uint i = 0; i < Rays.Length; i++)
                    Area.Lines[i] = new Line(Rays[i].End, Rays[((i == (Rays.Length - 1)) ? 0 : (i + 1))].End);
                return Area;
            }
        }
        ...
        public Raycaster(uint Rays)
        {
            this.Rays = new Ray[Math.Max(1, Rays)];
        }

        public void Cast(Vector2 Position, float Length, params Polygon[] Obstacles)
        {
            Cast(Position, Length, MathHelper.ToRadians(360), 0, Obstacles);
        }
        ...
        public void Cast(Vector2 Position, float Length, float Field, float Angle, params Polygon[] Obstacles)
        {
            ...
            for (uint i = 0; i < Rays.Length; i++)
            {
                ...
                foreach (var Obstacle in Obstacles)
                    if (Obstacle.Intersects(Rays[i], ref Rays[i].End)) Rays[i].Obstacle = Obstacle;
            }
            this.Rays = Rays;
        }

        public void DrawLines(Color? Color = null, float Size = 1, SpriteEffects Effect = SpriteEffects.None,
            float Layer = 0)
        {
            DrawLines(Globe.Batches[0], Color, Size, Effect, Layer);
        }
        ...
        public void DrawAreaOutline(Color Color, float Size = 1)
        {
            DrawAreaOutline(Globe.Batches[0], Color, Size);
        }
        ...
        {
            for (uint i = 0; i < Rays.Length; i++) if ((Rays[i] != null) && (Rays[i].Intersects(Polygon))) return true;
            return false;
        }
