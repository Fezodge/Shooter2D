...
namespace EzGame.Perspective.Planar
{
    public class Camera
    {
        ...
        public Matrix View
        {
            get
            {
                Matrix Matrix = (Matrix.CreateTranslation(new Vector3(-Position, 0)) * Matrix.CreateRotationZ(Angle) * Matrix.CreateScale(new Vector3(Zoom, Zoom, 1)) *
                    Matrix.CreateTranslation((Screen.BackBufferWidth / 2f), (Screen.BackBufferHeight / 2f), 0)), Invert = Matrix.Invert(Matrix);
