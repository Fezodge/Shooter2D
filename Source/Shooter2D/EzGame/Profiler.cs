...
namespace EzGame
{
    public static class Profiler
    {
        ...
        {
            get { return PrivateUpdate; }
            set
            {
                PrivateUpdate = value;
                if (!value && (Profiles != null))
                    foreach (var Key in Profiles.Keys) if (Profiles[Key].Stopwatch.IsRunning) Stop(Key);
            }
        }
        ...
        public static Profile Get(string Name, byte? Maximum)
        {
            ...
            Profiles.Add(Name, new Profile((Maximum ?? byte.MaxValue)) {Index = 0});
        ...
        {
            var Profile = Get(Name, null);
            var Index = ((Profile.Index ?? Profile.TimeSpans.Length) - 1);
            if (Index == -1) Index = (Profile.TimeSpans.Length - 1);
            return Profile.TimeSpans[Index].Ticks;
        }
        ...
        public static float AverageTicks(string Name)
        {
            var Profile = Get(Name, null);
            float Value = 0;
            for (var i = 0; i < Profile.Recorded; i++)
            {
                var TimeSpan = Profile.TimeSpans[i];
        ...
        public class Profile
        {
            ...
            public Profile(byte Maximum)
            {
                TimeSpans = new TimeSpan[Maximum];
            }
        }
    }
}