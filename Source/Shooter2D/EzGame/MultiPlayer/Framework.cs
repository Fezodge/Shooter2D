...
namespace EzGame
{
    public static class MultiPlayer
    {
        ...
        public static NetPeer Peer(string Name) { if (!string.IsNullOrEmpty(Name) && (Peers != null) && Peers.ContainsKey(Name)) return Peers[Name]; else return null; }
        private static string LastName;

        public enum Types { Server, Client }
        public static Types? Type() { return Type(LastName); }
        public static bool IsNullOrServer() { return IsNullOrServer(LastName); }
        public static Types? Type(string Name) { if (Peer(Name) is NetServer) return Types.Server; else if (Peer(Name) is NetClient) return Types.Client; else return null; }
        public static bool IsNullOrServer(string Name) { Types? Type = MultiPlayer.Type(Name); return ((Type == null) || (Type == Types.Server)); }

        public static void Initialize() { Peers = new Dictionary<string, NetPeer>(); }

        public static NetPeerConfiguration DefaultConfiguration
        {
            get
            {
                NetPeerConfiguration Configuration = new NetPeerConfiguration("Game");
        ...
        {
            LastName = Name;
            NetPeerConfiguration Configuration = DefaultConfiguration;
            Configuration.Port = Port;
            Configuration.MaximumConnections = MaximumConnections;
            return Start(Name, Configuration);
        }
        public static NetPeer Connect(string Name, string Host, int Port, NetPeerConfiguration Configuration, params object[] Variables)
        {
            LastName = Name;
        ...
        public static NetPeer Connect(string Name, string Host, int Port, params object[] Variables) { return Connect(Name, Host, Port, DefaultConfiguration, Variables); }

        public static NetIncomingMessage Read(string Name) { if (Peers.ContainsKey(Name) && (Peer(Name).Status == NetPeerStatus.Running)) return Peer(Name).ReadMessage(); else return null; }
        public static void Send(NetOutgoingMessage Message, NetDeliveryMethod Method = NetDeliveryMethod.ReliableOrdered, int Channel = 0)
        {
            if (Type(LastName) == Types.Server) { if (Peer(LastName).ConnectionsCount > 0) Peer(LastName).SendMessage(Message, Peer(LastName).Connections, Method, Channel); }
            else if (Type(LastName) == Types.Client) (Peer(LastName) as NetClient).SendMessage(Message, Method, Channel);
        }
        public static void Send(NetOutgoingMessage Message, NetConnection Except, NetDeliveryMethod Method = NetDeliveryMethod.ReliableOrdered, int Channel = 0)
        {
            if ((Except != null) && (Type(LastName) == Types.Server)) { if (Peer(LastName).ConnectionsCount > 0) (Peer(LastName) as NetServer).SendToAll(Message, Except, Method, Channel); }
            else Send(LastName, Message, Method, Channel);
        }
        public static void Send(string Name, NetOutgoingMessage Message, NetDeliveryMethod Method = NetDeliveryMethod.ReliableOrdered, int Channel = 0)
        {
            LastName = Name;
            if (Type(Name) == Types.Server) { if (Peer(Name).ConnectionsCount > 0) Peer(Name).SendMessage(Message, Peer(Name).Connections, Method, Channel); }
            else if (Type(Name) == Types.Client) (Peer(Name) as NetClient).SendMessage(Message, Method, Channel);
        }
        public static void Send(string Name, NetOutgoingMessage Message, NetConnection Except, NetDeliveryMethod Method = NetDeliveryMethod.ReliableOrdered, int Channel = 0)
        {
            LastName = Name;
            if ((Except != null) && (Type(Name) == Types.Server)) { if (Peer(Name).ConnectionsCount > 0) (Peer(Name) as NetServer).SendToAll(Message, Except, Method, Channel); }
            else Send(Name, Message, Method, Channel);
        }
        public static void SendTo(NetOutgoingMessage Message, NetConnection To, NetDeliveryMethod Method = NetDeliveryMethod.ReliableOrdered, int Channel = 0)
        {
            if (Type(LastName) == Types.Server) (Peer(LastName) as NetServer).SendMessage(Message, To, Method, Channel);
            else Send(LastName, Message, Method, Channel);
        }
        public static void SendTo(string Name, NetOutgoingMessage Message, NetConnection To, NetDeliveryMethod Method = NetDeliveryMethod.ReliableOrdered, int Channel = 0)
        {
            if (Type(Name) == Types.Server) (Peer(Name) as NetServer).SendMessage(Message, To, Method, Channel);
            else Send(Name, Message, Method, Channel);
        }
        public static void Flush(string Name) { if (Peer(Name) != null) Peer(Name).FlushSendQueue(); }
        public static void Shutdown(string Name, string Reason) { if (Peer(Name) != null) { Peer(Name).Shutdown(Reason); Peers.Remove(Name); } }

        /// <summary>
        /// Construct an outbound message without any built variables.
        /// </summary>
        /// <returns></returns>
        public static NetOutgoingMessage Construct() { return Peer(LastName).CreateMessage(); }
        /// <summary>
        ...
        {
            NetOutgoingMessage Message = Peer(LastName).CreateMessage();
            foreach (object Variable in Variables) Message.Write(Variable);
            return Message;
        }
        /// <summary>
        ...
        /// <returns></returns>
        public static NetOutgoingMessage Construct(string Name) { LastName = Name; return Peer(Name).CreateMessage(); }
        /// <summary>
        /// Construct an outbound message with the following variables.
        /// </summary>
        /// <param name="Name">The name of the peer to construct the message from.</param>
        ...
        {
            LastName = Name;
            NetOutgoingMessage Message = Peer(Name).CreateMessage();
            foreach (object Variable in Variables) Message.Write(Variable);
            return Message;
        }
        /// <summary>
        ...
        public static void Write(this NetBuffer Message, object Variable)
        {
            if (Variable is Array)
                for (int i = 0; i < (Variable as Array).Length; i++)
                {
                ...
                for (int i = 0; i < (Variable as List<object>).Count; i++)
                    Message.Write((Variable as List<object>)[i]);
            else if (Variable is List<byte>)
                for (int i = 0; i < (Variable as List<byte>).Count; i++)
        ...
        public static void Write(this NetBuffer Message, Point Variable) { Message.Write(Variable.X); Message.Write(Variable.Y); }
        public static Point ReadPoint(this NetBuffer Message) { return new Point(Message.ReadInt32(), Message.ReadInt32()); }
        public static void Write(this NetBuffer Message, Vector2 Variable) { Message.Write(Variable.X); Message.Write(Variable.Y); }
        public static Vector2 ReadVector2(this NetBuffer Message) { return new Vector2(Message.ReadFloat(), Message.ReadFloat()); }
        public static void Write(this NetBuffer Message, Vector3 Variable) { Message.Write(Variable.X); Message.Write(Variable.Y); Message.Write(Variable.Z); }
        public static Vector3 ReadVector3(this NetBuffer Message) { return new Vector3(ReadVector2(Message), Message.ReadFloat()); }
        public static void Write(this NetBuffer Message, Vector4 Variable) { Message.Write(Variable.X); Message.Write(Variable.Y); Message.Write(Variable.Z); Message.Write(Variable.W); }
        public static Vector4 ReadVector4(this NetBuffer Message) { return new Vector4(ReadVector3(Message), Message.ReadFloat()); }
        public static void Write(this NetBuffer Message, Line Variable) { Write(Message, Variable.Start); Write(Message, Variable.End); }
        public static Line ReadLine(this NetBuffer Message) { return new Line(ReadVector2(Message), ReadVector2(Message)); }
        public static void Write(this NetBuffer Message, Rectangle Variable) { Message.Write(Variable.X); Message.Write(Variable.Y); Message.Write(Variable.Width); Message.Write(Variable.Height); }
        ...
        public static void Write(this NetBuffer Message, BoundingSphere BoundingSphere) { Write(Message, BoundingSphere.Center); Write(Message, BoundingSphere.Radius); }
        ...
        { return new Quaternion(Message.ReadSignedSingle(Bits), Message.ReadSignedSingle(Bits), Message.ReadSignedSingle(Bits), Message.ReadSignedSingle(Bits)); }
        public static void Write(this NetBuffer Message, Matrix Variable)
        {
            Quaternion Quaternion = Quaternion.CreateFromRotationMatrix(Variable);
        ...
        {
            Quaternion Quaternion = ReadQuaternion(Message, 24);
        ...
        {
            Polygon Polygon = new Polygon(new Line[Message.ReadByte()]);
